...
vector<Process> readWorkloadFile(const string& filePath) {
    ...
    while (getline(infile, line)) {
        ...
        // Initialize remaining CPU burst
...
void fifoScheduling(vector<Process>& processes, int numProcessors) {
    int currentTime = 0;
    vector<deque<int>> readyQueues(numProcessors);
    vector<int> processorTimes(numProcessors, 0);
    int numProcesses = processes.size();
    int processesCompleted = 0;

    while (processesCompleted < numProcesses) {
        // Add new arrivals to the ready queues
        for (int i = 0; i < numProcesses; i++) {
            if (processes[i].arrivalTime <= currentTime && !processes[i].inIO && !processes[i].completed) {
                bool added = false;
                for (int j = 0; j < numProcessors; j++) {
                    if (find(readyQueues[j].begin(), readyQueues[j].end(), i) == readyQueues[j].end()) {
                        readyQueues[j].push_back(i);
                        added = true;
                        break;
                    }
                }
                if (!added) {
                    // If no processor had space, add to the first one
        ...
        // Process each processor's ready queue
        for (int j = 0; j < numProcessors; j++) {
            if (!readyQueues[j].empty()) {
                int processID = readyQueues[j].front();
                readyQueues[j].pop_front();

                Process& process = processes[processID];
                cout << "Executing Process " << processID + 1 << " on Processor " << j + 1
                     << ", CPU Burst " << process.currentCpuBurst + 1 << endl;

                processorTimes[j] += process.cpuBursts[process.currentCpuBurst];
                currentTime = max(currentTime, processorTimes[j]);
                ...
                if (process.currentCpuBurst < process.cpuBursts.size()) {
                    ...
                    // Debug: Print I/O state
                    cout << "Process " << processID + 1 << " enters I/O, I/O Completion Time: " << process.ioCompletionTime << endl;
                } else {
                    process.completionTime = currentTime;
                    process.completed = true;
                    processesCompleted++;
                    // Debug: Print completion state
                    cout << "Process " << processID + 1 << " completed, Completion Time: " << process.completionTime << endl;
                }
            }
        }

        // Handle idle time
        if (all_of(readyQueues.begin(), readyQueues.end(), [](const deque<int>& q) { return q.empty(); })) {
    ...
    // Debug: Print final process metrics
    cout << "Final Process Metrics:" << endl;
    cout << "Process ID\tArrival Time\tTotal CPU Burst\tCompletion Time\tTAT\tWT" << endl;
    for (int i = 0; i < numProcesses; i++) {
        ...
        cout << "P" << i + 1 << "\t\t" << process.arrivalTime << "\t\t" 
             << process.totalCpuBurstTime << "\t\t" << process.completionTime 
             << "\t\t" << process.turnaroundTime << "\t" << process.waitingTime << endl;
    }

    // Compute and print average metrics
    double averageTAT = 0.0, averageWT = 0.0;
    ...
    cout << "Average Turnaround Time (ATAT): " << averageTAT << endl;
...
void roundRobinScheduling(vector<Process>& processes, int timeQuantum) {
    ...
    while (processesCompleted < numProcesses) {
        ...
        if (!readyQueue.empty()) {
            ...
            Process& process = processes[processID];
            int timeSlice = min(timeQuantum, remainingTime[processID]);

            cout << "Executing Process " << processID + 1 << " for " << timeSlice << " units\n";
            currentTime += timeSlice;
            remainingTime[processID] -= timeSlice;

            // If process completed its CPU burst
            if (remainingTime[processID] == 0) {
                process.currentCpuBurst++;
                if (process.currentCpuBurst < process.cpuBursts.size()) {
                    // Move to I/O
                    process.inIO = true;
            ...
            // Re-add to the ready queue if not completed
            if (!process.completed && remainingTime[processID] > 0) {
                readyQueue.push_back(processID);
            }
        } else {
            // Advance time to the next event
